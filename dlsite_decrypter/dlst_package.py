...
class DlstPackage(KaitaiStruct):
    ...
    class DlstStorage(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.filename = self._io.read_bytes(self.filename_len * 2).decode('UTF-16-LE')
    ...
    def package_footer(self):
        ...
        self._m_package_footer = self._root.DlstPackageFooter(self._io, self, self._root)
        self._io.seek(_pos)
        if hasattr(self, '_m_package_footer'):
            return self._m_package_footer
    ...
    def package_storage(self):
        ...
        for i in range(self.package_footer.file_count):
            self._m_package_storage[i] = self._root.DlstStorage(self._io, self, self._root)
